...
std::string CogitorCredentials::makeOfflineStatusJson(const char* reason)
{
    return std::format(R"({{"online":false,"ts":0,"reason":"{}"}})",
                       reason ? reason : "lwt");
}

CogitorCredentials::CogitorCredentials(std::string productId,
                                       std::string deviceId,
                                       std::string username,
                                       std::string password,
                                       std::string clientId,
...
std::optional<CogitorCredentials> CogitorCredentials::build(
    ...
    bool accessTokenAuth)
{
    ...
    return CogitorCredentials(std::string(productId),
                              std::string(deviceId),
                              std::string(username),
                              std::string(password),
                              std::move(clientId),
...
std::optional<CogitorCredentials> CogitorCredentials::createAccessToken(
    std::string_view productId,
    std::string_view deviceId,
    std::string_view host,
...
std::optional<CogitorCredentials> CogitorCredentials::createBasic(
    std::string_view productId,
    std::string_view deviceId,
    std::string_view host,
    std::string_view username,
