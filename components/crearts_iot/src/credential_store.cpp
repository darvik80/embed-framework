#include "crearts_iot/credential_store.hpp"
...
static const char* TAG = "CreartsCredsNvs";
...
std::optional<CreartsCredentials> loadOrSeedCredentials(
    ...
    uint16_t seedPort)
{
    embed::CreartsSettings s{};
    const bool fromNvs = embed::loadCreartsSettings(s) && embed::creartsSettingsComplete(s);
    ...
    if (!fromNvs) {
        ...
        const esp_err_t err = embed::saveCreartsSettings(s);
        if (err != ESP_OK) {
    ...
    return CreartsCredentials::createAccessToken(
