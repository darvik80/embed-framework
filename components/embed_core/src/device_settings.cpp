...
FactoryResetHandler s_factoryResetHandler = nullptr;
...
bool loadWifiFrom(const char* ns, WifiSettings& out)
{
    out = {};
    NvsStore store;
...
esp_err_t saveWifiTo(const char* ns, const WifiSettings& in)
{
    NvsStore store;
    esp_err_t err = store.open(ns);
    if (err != ESP_OK) {
...
{
    return saveWifiTo(kWifiBakNs, in);
}

esp_err_t eraseWifiSettings()
{
    return eraseNamespace(kWifiNs);
}

esp_err_t backupWifiSettings()
{
    WifiSettings w{};
    if (!loadWifiSettings(w)) {
        return ESP_ERR_NOT_FOUND;
    }
    return saveWifiBackup(w);
}
...
{
    s_factoryResetHandler = handler;
}

esp_err_t factoryResetSettings()
{
    if (s_factoryResetHandler) {
        return s_factoryResetHandler();
    }
    ESP_LOGW(TAG, "factory reset WiFi settings");
    const esp_err_t bak = backupWifiSettings();
...
#else
static constexpr int kResetGpio = 0;
#endif
...
static constexpr uint32_t kRstBurstMagic = 0x52535421;
