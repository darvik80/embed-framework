...
components/cogitor_iot/# Cogitor IoT Platform device SDK (preferred cloud path; owns MQTT NVS + config portal)
...
Use `embed/crypto.hpp` for SHA-256 / MD5 / HMAC — works on ESP-IDF 5.x (legacy mbedtls) and 6.x (PSA Crypto); do not include `mbedtls/sha256.h` directly.
Secrets (WiFi password, Cogitor access token) → NVS `fctry` (seeded from `sdkconfig` / menuconfig on first boot), not source literals. WiFi + portal flag: `embed_core` `device_settings`. Cogitor identity + config portal + credentials JSON: `cogitor_iot` (`CogitorSettings`, `ConfigPortalService`, `factoryResetSettings`). RPC `factory_reset` / `config_portal` / `import_credentials`. OTA rollback: keep image `PENDING_VERIFY` until MQTT; `checkCrashLoopRollback()` early in `app_main` (bootloader rollback + 3 failed boots / panic-WDT).
